...
for participant in participants:
    ...
    phasediffs_no_tacs = np.array(phasediffs)[:n_trials]
    ...
    phasediffs_tacs_without_sass = np.array(phasediffs)[:n_trials]
    ...
    phasediffs_no_tacs_with_sass = np.array(phasediffs)[:n_trials]
...
plt.tight_layout()
sns.despine()
plt.savefig('figs/open/group_amplitudes.pdf')
...
np.save('figs/open/pvals_group_amplitudes.npy',pvals)
...
plt.tight_layout()
sns.despine()
plt.savefig('figs/open/group_plvs.pdf')
...
pvals = np.array([p1,p2,p3])
np.save('figs/open/pvals_group_plvs.npy',pvals)